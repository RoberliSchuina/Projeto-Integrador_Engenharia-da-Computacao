...
Este repositório contém o código-fonte e a documentação do protótipo de um sistema de controle de acesso RFID que também gerencia o consumo de energia de uma sala de aula (ou escritório), com o objetivo de reduzir o desperdício de energia elétrica.
...
Controle de Acesso: Validação de usuários via cartões/tags RFID (MFRC522).

Gestão de Energia: Acionamento de um Relé 5V (simulando a rede elétrica da sala) apenas para usuários autorizados.

Feedback Visual Unificado: Uso de um LED RGB para indicar o status do sistema:

Azul: Ocioso (Aguardando cartão)

Verde: Acesso Liberado (Energia ligada)

Vermelho: Acesso Negado

Atuadores Físicos: Controle de um Servo Motor (simulando uma cancela ou fechadura elétrica).

Relógio em Tempo Real (RTC): O sistema utiliza um módulo RTC DS1302 para registrar data/hora de cada acesso e permitir futuras lógicas de "horário de funcionamento".

Display de Status: Um LCD 16x2 exibe informações em tempo real:

Modo Ocioso: Alterna entre o nome do projeto e o relógio (Data/Hora).

Modo de Ação: Exibe "Acesso Liberado" (com data/hora do evento) ou "Acesso Negado".
...
No arquivo .ino, localize a função setup().

Descomente a linha: // Rtc.SetDateTime(RtcDateTime(2025, 11, 7, 11, 45, 0));
...
No código, encontre o loop() e adicione o UID à condição de acesso liberado:
